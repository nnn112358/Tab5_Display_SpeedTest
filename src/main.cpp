...
void showResults() {
  ...
  for (int i = 0; i < NUM_SIZES; i++) {
    Serial.print("\nSize: ");
    Serial.println(SCREEN_SIZES[i].name);
    for (int r = 0; r < NUM_ROTATIONS; r++) {
      if (tested[i][r]) {
        Serial.print("Rotation ");
        Serial.print(r);
        Serial.print(": ");
        Serial.print(results[i][r]);
  ...
  Serial.println("\nMAXIMUM SIZE TEST");
  Serial.print("Size: ");
  Serial.print(maxSize.width);
  Serial.print("x");
  Serial.println(maxSize.height);
...
bool shouldSkip() {
  ...
  if (w == 720 && h == 1280 && (rotMode == 1 || rotMode == 3)) {
    Serial.printf("Skipping size %dx%d with rotation %d (as requested)\n", w, h,
                  rotMode);
    return true;
  }
  ...
  //  Serial.printf("Skipping size %dx%d (exceeds display size %dx%d)\n", w, h,
...
void setupNextTest() {
    ...
    if (rotMode >= NUM_ROTATIONS) {
      ...
      if (!inMaxTest) {
        ...
        if (sizeIndex >= NUM_SIZES) {
          ...
          inMaxTest = true;
          Serial.println("Starting maximum size test");
        }
      } else {
  ...
  if (!inMaxTest) {
    Serial.print(SCREEN_SIZES[sizeIndex].name);
  } else {
    Serial.print(maxSize.width);
    Serial.print("x");
    Serial.print(maxSize.height);
  }
  Serial.print(", Rotation ");
...
void setup() {
  ...
  Serial.println("Starting display speed test...");
  setupNextTest();
}
